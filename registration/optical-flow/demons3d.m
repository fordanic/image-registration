function [displacementField, certainty] = demons3d(moving, fixed, varargin)
% DEMONS3D Estimates a displacement field using the original demons scheme
%
...
displacementField{3} = zeros(size(moving));

switch method
    case 'moving'
        % Estimate gradient of moving image
        [movingGradX movingGradY movingGradZ] = gradient(fixed);
        gradX = (movingGradX);
        gradY = (movingGradY);
        gradZ = (movingGradZ);
    case 'fixed'
        % Estimate gradient of fixed image
        [fixedGradX fixedGradY fixedGradZ] = gradient(fixed);
        gradX = (fixedGradX);
        gradY = (fixedGradY);
        gradZ = (fixedGradZ);
    case 'symmetric'
        ...
        gradZ = 0.5*(fixedGradZ + movingGradZ);
end
...
denominator = numerator.^2 + (gradX.^2 +  gradY.^2 + gradZ.^2) + eps;

displacementField{1} = (numerator.*gradX)./denominator;
displacementField{2} = (numerator.*gradY)./denominator;
displacementField{3} = (numerator.*gradZ)./denominator;

currentMaxDisplacement = max(sqrt(displacementField{1}(:).^2 + displacementField{2}(:).^2 + displacementField{3}(:).^2));
...
if currentMaxDisplacement > maxDisplacement
    ...
    displacementField{3} = displacementField{2}/normalizingFactor;
end

certainty = sqrt(gradX.^2 +  gradY.^2 + gradZ.^2);