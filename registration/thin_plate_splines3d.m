function [output] = thin_plate_splines3d(input, outputSize, ...
...
W = inv(L)*Y; % (W|a1 ax ay az)' = inv(L)*Y

% Thin-plate spline mapping (Map all points in the plane)
[Xw, Yw, Zw] = tpsMap(W, height, width, depth, Xi, Yi, Zi, NPs);
...
%%
function [Xw, Yw, Zw] = tpsMap(W, height, width, depth, Xi, Yi, Zi, NPs)
